          ...
          <div className={`z-[5] grid items-center md:px-4 `}>
              <div className=" my-4 grid px-4 sm:grid-cols-2  md:space-x-4 ">
               ...
               border-${data?.color}-500 bg-${data?.color}-500`}
                />
                <div className="grid w-full justify-center items-center md:mr-4 md:space-y-4">
                <p className="text-content my-4 break-normal text-center text-4xl uppercase  md:my-0 ">
                  ...
                  <p className="text-content my-4 break-normal text-center text-xs uppercase  md:my-0 ">
                    Suggested By:
                    <br /> <span className="text-text/50">{data?.suggestedBy}</span>
                  </p>
                  <button className={`group mx-auto h-10 max-w-fit rounded-md text-text hover:bg-opacity-80 disabled:bg-slate-300 disabled:text-slate-500 dark:disabled:bg-slate-700 dark:disabled:text-slate-500  bg-${data?.color}-400 dark:bg-${data?.color}-600 px-3 py-1 text-sm xl:text-base`}
                  onClick={handleClick}>
                    Checkout!
                    </button>
...
export const CheckoutSuccess = () => {
  ...
  return (
    <section
      ...
      className="z-10 grid h-full flex-1 bg-bg w-full justify-normal text-text lg:mx-auto lg:min-w-[900px] lg:max-w-[900px] "
    >
      <div className="relative h-min overflow-hidden border-t-2 py-8  dark:border-none dark:shadow  md:mb-4 md:rounded-b-md  md:border-none md:shadow-xl  ">
        ...
        >
          ...
          <p className=" break-all w-full text-text/60 text-center"> {sessionId} </p>
