...
//    edit the modpack data from packDetails using a mutation and queryClient to invalidate the cache and update the data on the page without a refresh 
    ...
    const [modpackDescription, setModpackDescription] = React.useState<string>(packDetails?.description)
    const [modpackImage, setModpackImage] = React.useState<File>()
    const [modpackColor, setModpackColor] = React.useState<string>(packDetails?.color)
    const [modpackSuggestor, setModpackSuggestor] = React.useState<string>(packDetails?.suggestedBy)
    ...
    return (
      <>
        ...
        <div className='flex items-center justify-center'>
          <h1 className='text-2xl xl:text-3xl m-3 mt-5'>Edit </h1>
        </div>
        <form  method="post" className='grid items-center placeholder:text-slate-400 justify-center gap-4 dark:text-bg pt-[.5em]  text-sm xl:text-base' onSubmit={ async(e: React.FormEvent<HTMLFormElement>)=>
          {
            ...
            editModpackMutation.mutate({modpackName, modpackDescription,modpackImage,modpackColor,modpackSuggestor})
          }} >
          
          {/* Modpack name field, single line. */}
          <input required className={` h-8 rounded-md border-2  border-${packDetails?.color}-500 px-3 py-1`} type="text" placeholder="Modpack Name" value={modpackName} onChange={(e) => setModpackName(e.target.value)} />
          ...
          <textarea
            className={` min-h-[100px] rounded-md border-2   border-${packDetails?.color}-500 px-3 py-1 w-96 out-of-range:border-red-500 `}
          ...
          <input required name='modpack__image--input' className={`rounded-md border-2 dark:text-text cursor-pointer file:placeholder:text-slate-400 border-${packDetails?.color}-500 w-full h-8 px-3 py-1`} type="file" onChange={fileSelectedHandler} />
          ...
          <input  className={`h-8 rounded-md border-2   border-${packDetails?.color}-500 px-3 py-1 `} type="text" placeholder="Modpack Suggestor" value={modpackSuggestor} onChange={(e) => setModpackSuggestor(e.target.value)} />

          <br/>
          
          <button className={`dark:text-bg  h-16 rounded-md border-2 border-black bg-${packDetails?.color}-500 px-3 py-1 text-sm xl:text-base`}>
          Edit Modpack
          </button>
