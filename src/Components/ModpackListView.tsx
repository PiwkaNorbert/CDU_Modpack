  ...
  return (
    <>
      ...
      {isLoading ? (
        ...
        <div className="my-10 text-center">
          <div>
            ...
            <p>{(error?.response?.data as { error?: string })?.error ?? ""}</p>
          </div>
        </div>
      ...
      data?.length === 0 || !Array.isArray(data) ? (
        <div className=" my-10 flex-1 text-center capitalize ">
          ...
          {(user?.isLinked || user?.isAdmin) &&
            !(
              ...
              <div className="z-[2] grid grid-cols-2 gap-5 p-5 max-[400px]:grid-cols-1 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5   ">
                <SuggestPackCard />
              </div>
            )}
        </div>
        ...
        <>
          <div className="z-[2] grid grid-cols-2 gap-x-5 gap-y-10 py-10 max-[400px]:grid-cols-1 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 ">
            {(user?.isLinked || user?.isAdmin) &&
              !(
                location.pathname.includes("archived") ||
                location.pathname.includes("suggested")
